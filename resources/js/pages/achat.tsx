...
function ItemCard({ item }: { item: Item }) {
    ...
    function inc() {
        setQuantite((q) => Math.min(item.stock, q + 1));
    }
            ...
            <div className="flex flex-1 flex-col p-4">
                ...
                <div className="mt-auto pt-3">
                    <div className="mb-3 flex items-center justify-between border-t border-[#1a1a1a] pt-3">
                        <div>
                            <div className="text-[10px] text-[#444]">PRIX UNITAIRE</div>
                            <div className="text-lg font-bold text-[#00ff41]">
                                {item.prix_vente ? `$${Number(item.prix_vente).toLocaleString()}` : 'N/A'}
                            </div>
                        </div>
                        {item.stock > 0 && quantite > 1 && (
                            <div className="text-right">
                                <div className="text-[10px] text-[#444]">TOTAL</div>
                                <div className="text-sm font-bold text-[#00ff41]">
                                    ${(Number(item.prix_vente) * quantite).toLocaleString()}
                                </div>
                            </div>
                        )}
                    </div>
